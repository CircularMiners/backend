...
router.get("/searchmine", async (req: Request, res: Response) => {
  ...
  const guid =
    ID && typeof ID.toString() === "string" ? Guid.parse(ID.toString()) : null;
  const name = req.query.mineName ? req.query.mineName : "";
  const location = req.query.mineLocation ? req.query.mineLocation : "";
  const description = req.query.mineDescription
    ? req.query.mineDescription
    : "";
  const representativeID = req.query.mineRepresentativeID
    ? req.query.mineRepresentativeID
    : "";
  const gRepresentativeID =
    representativeID && typeof representativeID.toString() === "string"
      ? Guid.parse(representativeID.toString())
      : null;
